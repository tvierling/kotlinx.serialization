...
class JsonSchemaCreator : DescriptorTraveller<JsonObject> {
    ...
    override fun visitChild(desc: SerialDescriptor, child: SerialDescriptor, index: Int): Boolean {
        val elementName = desc.getElementName(index)
        properties[elementName] = child.traverse(JsonSchemaCreator())
        if (!desc.isElementOptional(index)) requiredProperties.add(elementName)
